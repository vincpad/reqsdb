...
class reqDb:
    ...
    def addProject(self, name, prefix, description, username):
        Projects(name=name, prefix=prefix, description=description).save()
        self.addType(name=name, prefix=prefix, description=description, category_name="PROJECT", project_prefix=prefix)
        self.addItem(name, description=description, type=prefix, project_prefix=prefix, user=username)
    ...
    def addItem(self, name, description, type, project_prefix, user, set_key_prefix = None):
        ...
        newversion = Versions(item_id = newitem, fields_id = newfield, user_id=_user, status=status, justification="Creation")
        newitem.save()
        newfield.save()
        newversion.save()
        return localId
    ...
    def updateItem(self, justification, local_id, name, description, username, status):
        ...
        newversion = Versions(item_id = item, fields_id = newfield, user_id=user, status=_status, justification=justification, number=numberOfVersions+1)
    ...
    def getChildren(self, parentLocalId, type):
        parent = Items.get(Items.local_id == parentLocalId)
        typeId = RelationsTypes.get(RelationsTypes.name == type)
        childRelations = list(Relations.select(Relations.child_id).where(Relations.parent_id == parent, Relations.type_id == typeId))
    ...
    def getParents(self, childLocalId, type):
        child = Items.get(Items.local_id == childLocalId)
        typeId = RelationsTypes.get(RelationsTypes.name == type)
        parentRelations = list(Relations.select(Relations.parent_id).where(Relations.child_id == child, Relations.type_id == typeId))
    ...
    def addFolder(self,name, description, user, sourceItemLocalId, project_prefix = "OB1"):
        newItemLocalId = self.addItem(name=name, description=description, type="FLD",project_prefix=project_prefix, user=user, set_key_prefix=rdb.getSetKeyPrefix(sourceItemLocalId))
...
def initAll():
    ...
    newSet = rdb.addSetOfRequirements(name="Stakeholders Needs", prefix="STK", description="Stakeholders needs", project="OB1", user="vrocher")
    rdb.addRelation("OB1-OB1-1", newSet, "HIERARCHY")
    
    newItem = rdb.addItem("Being profitable", "Make money in other terms", "STK", "OB1", "vrocher")
    rdb.addRelation(newSet, newItem, "HIERARCHY")
